...
func createValidationBenchmarkSuite() *BenchmarkSuite {
	...
	suite := NewBenchmarkTable("ValidationLibrary").
		...
		WithCase("Range_Int_Invalid", wrapRangeIntValidator(1, 100), 150).
		WithCase("Range_Int64_Valid", wrapRangeInt64Validator(1, 100), int64(50)).
		WithCase("Range_Int64_Invalid", wrapRangeInt64Validator(1, 100), int64(150)).
		WithCase("Range_Float64_Valid", wrapRangeFloat64Validator(0.0, 100.0), 50.5).
		WithCase("Range_Float64_Invalid", wrapRangeFloat64Validator(0.0, 100.0), 150.5).
		
		// String Length Validators  
		WithCase("StringLength_MinLen_Valid", wrapStringLengthValidator("minlen", 5, 0), "hello world").
		WithCase("StringLength_MinLen_Invalid", wrapStringLengthValidator("minlen", 5, 0), "hi").
		WithCase("StringLength_MaxLen_Valid", wrapStringLengthValidator("maxlen", 0, 10), "hello").
		WithCase("StringLength_MaxLen_Invalid", wrapStringLengthValidator("maxlen", 0, 10), "this is too long").
		...
		WithCase("Characteristic_Alpha_Invalid", wrapCharacteristicValidator(rules.RuleStaticAlpha), "hello123").
		WithCase("Characteristic_AlphaNumeric_Valid", wrapCharacteristicValidator(rules.RuleStaticAlphaNumeric), "hello123").
		WithCase("Characteristic_AlphaNumeric_Invalid", wrapCharacteristicValidator(rules.RuleStaticAlphaNumeric), "hello@123").
		WithCase("Characteristic_Numeric_Valid", wrapCharacteristicValidator(rules.RuleStaticNumeric), "12345").
		WithCase("Characteristic_Numeric_Invalid", wrapCharacteristicValidator(rules.RuleStaticNumeric), "123abc").
		
		// OneOf Validators
		WithCase("OneOf_Valid", wrapOneOfValidator([]string{"red", "green", "blue"}), "blue").
		WithCase("OneOf_Invalid", wrapOneOfValidator([]string{"red", "green", "blue"}), "yellow").
...
func ExampleOtherLibraryBenchmark(b *testing.B) {
